...
class SMTP
{
    ...
    public function connect($host, $port = null, $timeout = 30, $options = [])
    {
        ...
        if ($this->smtp_conn === false) return false;
    ...
    {
        static $streamok;
        if (null === $streamok) $streamok = function_exists('stream_socket_client');
        $errno = 0;
        $errstr = '';
        if ($streamok) {
    ...
    public function authenticate($username, $password, $authtype = null, $OAuth = null)
    {
        ...
        switch ($authtype) {
            case 'PLAIN':
                if (!$this->sendCommand('AUTH', 'AUTH PLAIN', 334)) return false;
    ...
    {
        if ($this->sendHello('EHLO', $host)) return true;
        ...
        foreach ($lines as $n => $s) {
            ...
            if (!empty($fields)) {
                if (!$n) {
                    ...
                    switch ($name) {
                        case 'SIZE': $fields = ($fields ? $fields[0] : 0); break;
                        case 'AUTH': if (!is_array($fields)) $fields = []; break;
                        default: $fields = true;
                    }
                }
    ...
    public function xclient(array $vars)
    {
        ...
        if (!$xclient_options) return true;
    ...
    public function setSMTPUTF8($enabled = false) { $this->do_smtputf8 = $enabled; }
    public function getSMTPUTF8() { return $this->do_smtputf8; }
